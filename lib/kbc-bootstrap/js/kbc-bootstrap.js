...
"use strict"
    ...
    ns.initialize = function(opts){
        ...
        var $linkers = $("a.inner-scroll[href^=#]");
        for(var i = 0 ; i < $linkers.length; i++){
            var $linker = $($linkers[i]);
            $linker.click((kbc.util.smoothScroll).bind($linker));
        }
    ...
    var makeDateString = function(dateObj, noneText){
        var date;
    ...
    ns.event = function($elem, opts, headlineClass){
        if(!$elem){
            return;
        }
        opts = kbc.util.applyDefaultOpts(opts, {
        ...
        var date = makeDateString(opts.date, "(未定)");
        var fee;
        ...
        var deadline = makeDateString(opts.deadline, "特になし");

        var $headline = $("<h2>").addClass("headline")
                                 .append(opts.name);
        $elem.append($("<div>")
                     .addClass("kbc-event")
                     .append($headline)
        ...
        kbc.controller.appendSidemenu($headline, headlineClass);
    };
}(this, "kbc", "view"));
...
"use strict"
    ...
    ns.appendSidemenu = function($headline, classes){
